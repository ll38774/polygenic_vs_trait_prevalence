...
height_csv <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/NCD_RisC_Lancet_2020_height_child_adolescent_country.csv"
map_path  <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/pop_to_country.tsv"
out_dir   <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/results_height_pgs"
...
# ====== 1) 读取并清洗身高数据（2013年，18-19岁，男女分开） ======
h_raw <- read_csv(height_csv, show_col_types = FALSE)

# 把所有以 "Mean height" 开头的列合并为一列 Height_cm（容错处理）
mh_cols <- grep("^Mean height", names(h_raw), value = TRUE)
stopifnot(length(mh_cols) >= 1)
h_tidy <- h_raw %>%
  mutate(
    Height_cm = do.call(coalesce, c(across(all_of(mh_cols)), list(NA_real_)))
  ) %>%
  select(Country, Sex, Year, `Age group`, Height_cm) %>%
  filter(Year == 2013, `Age group` %in% c(18, 19)) %>%
  group_by(Country, Sex) %>%
  summarise(Height_cm = mean(Height_cm, na.rm = TRUE), .groups = "drop")
...
dat <- pgs_by_sex %>%
  inner_join(h_tidy, by = c("Country", "Sex")) %>%
  select(Sex, SuperPop, Population, Country, Mean_PGS, Height_cm)

# ====== 5a) 计算全局 z-score（所有群体合并） ======
dat_global <- dat %>%
  mutate(PGS_z = as.numeric(scale(Mean_PGS)))  # 全局标准化

# ====== 5b) 计算组内 z-score（每个 super-pop 内部标准化） ======
dat_within <- dat %>%
  group_by(SuperPop) %>%
...
# 定义一个通用回归函数
fit_one <- function(df) {
  model <- lm(Height_cm ~ PGS_z, data = df)
  s <- summary(model)
  tibble(
    n = nrow(df),
...
# 保存
write_csv(res_all, file.path(out_dir, "height_pgs_regression_results.csv"))
...
stats_global <- dat_global %>%
  ...
  dplyr::do({
    df <- .
    m <- lm(Height_cm ~ PGS_z, data = df)
    s <- summary(m)
    tibble::tibble(
      beta1 = unname(coef(m)["PGS_z"]),
      p = s$coefficients["PGS_z", "Pr(>|t|)"],
      r = cor(df$PGS_z, df$Height_cm, use = "complete.obs"),
...
# 2) 画图（仅一条全局回归线）+ 右上角标注
p_global <- dat_global %>%
  ggplot(aes(x = PGS_z, y = Height_cm, color = SuperPop, label = Population)) +
  geom_point(size = 1.5) +
  ...
  labs(x = "PGS (global z-score)", y = "Mean height (cm)",
       title = "Global regression: Height vs PGS (z, all populations combined)") +
  theme_bw()

ggsave(file.path(out_dir, "height_vs_pgs_global_annotated.png"),
...
for (sp in unique(dat_within$SuperPop)) {
  ...
  stats_within <- df_sp %>%
    ...
    dplyr::do({
      ...
      if (nrow(d) < 3) {
        ...
        m <- lm(Height_cm ~ PGS_z, data = d)
        s <- summary(m)
        tibble::tibble(
          beta1 = unname(coef(m)["PGS_z"]),
          p = s$coefficients["PGS_z", "Pr(>|t|)"],
          r = cor(d$PGS_z, d$Height_cm, use = "complete.obs"),
  ...
  p_sp <- df_sp %>%
    ggplot(aes(x = PGS_z, y = Height_cm, label = Population)) +
    geom_point(size = 1.5) +
    geom_text_repel(aes(label = Population), size = 2, max.overlaps = 50, show.legend = FALSE)+
    ...
    labs(x = paste0("PGS within ", sp, " (z-score)"),
         y = "Mean height (cm)",
         title = paste0("Within ", sp, ": Height vs PGS (z)")) +
    theme_bw()
  
  ggsave(file.path(out_dir, paste0("height_vs_pgs_within_", sp, "_annotated.png")),
