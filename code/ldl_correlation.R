...
ldl_csv <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/NCD_RisC_Nature_2020_Cholesterol_age_standardised_countries.csv"
map_path  <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/pop_to_country.tsv"
out_dir   <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/results_ldl_pgs"
...
ldl_raw <- read_csv(ldl_csv, show_col_types = FALSE)

ldl_tidy <- ldl_raw %>%
  rename(Country = `Country/Region/World`) %>%
  filter(Year == 2013) %>%
  select(Country, Sex, LDL = `Mean non-HDL cholesterol (mmol/L)`) %>%
  mutate(Sex = recode(Sex, "Men" = "Boys", "Women" = "Girls"))
...
dat <- pgs_by_sex %>%
  inner_join(ldl_tidy, by = c("Country", "Sex")) %>%
  select(Sex, SuperPop, Population, Country, Mean_PGS, LDL)
...
# 保存
write_csv(res_all, file.path(out_dir, "ldl_pgs_regression_results.csv"))
...
stats_global <- dat_global %>%
  ...
  dplyr::do({
    df <- .
    m <- lm(LDL ~ PGS_z, data = df)
    s <- summary(m)
    tibble::tibble(
      beta1 = unname(coef(m)["PGS_z"]),
      p = s$coefficients["PGS_z", "Pr(>|t|)"],
      r = cor(df$PGS_z, df$LDL, use = "complete.obs"),
...
# 2) 画图（仅一条全局回归线）+ 右上角标注
p_global <- dat_global %>%
  ggplot(aes(x = PGS_z, y = LDL, color = SuperPop, label = Population)) +
  geom_point(size = 1.5) +
  ...
  labs(x = "PGS (global z-score)", y = "LDL cholesterol(mmol/L)",
       title = "Global regression: LDL cholesterol vs PGS (z, all populations combined)") +
  theme_bw()

ggsave(file.path(out_dir, "ldl_vs_pgs_global_annotated.png"),
...
for (sp in unique(dat_within$SuperPop)) {
  ...
  stats_within <- df_sp %>%
    ...
    dplyr::do({
      ...
      if (nrow(d) < 3) {
        ...
        m <- lm(LDL ~ PGS_z, data = d)
        s <- summary(m)
        tibble::tibble(
          beta1 = unname(coef(m)["PGS_z"]),
          p = s$coefficients["PGS_z", "Pr(>|t|)"],
          r = cor(d$PGS_z, d$LDL, use = "complete.obs"),
  ...
  p_sp <- df_sp %>%
    ggplot(aes(x = PGS_z, y = LDL, label = Population)) +
    geom_point(size = 1.5) +
    geom_text_repel(aes(label = Population), size = 2, max.overlaps = 50, show.legend = FALSE)+
    ...
    labs(x = paste0("PGS within ", sp, " (z-score)"),
         ...
         title = paste0("Within ", sp, ": LDL cholesterol vs PGS (z)")) +
    theme_bw()
  
  ggsave(file.path(out_dir, paste0("ldl_vs_pgs_within_", sp, "_annotated.png")),
