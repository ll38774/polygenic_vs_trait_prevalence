...
bp_csv <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/NCD_RisC_Lancet_2017_BP_age_standardised_countries.csv"
map_path  <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/pop_to_country.tsv"
out_dir   <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/results_sbp_pgs"
...
bp_raw <- read_csv(bp_csv, show_col_types = FALSE)

bp_tidy <- bp_raw %>%
  rename(Country = `Country/Region/World`) %>%
  filter(Year == 2013) %>%
  select(Country, Sex, SBP = `Mean systolic blood pressure (mmHg)`) %>%
  mutate(
...
dat <- pgs_by_sex %>%
  inner_join(bp_tidy, by = c("Country", "Sex")) %>%
  select(Sex, SuperPop, Population, Country, Mean_PGS, SBP)
...
stats_global <- dat_global %>%
  ...
  dplyr::do({
    df <- .
    m <- lm(SBP ~ PGS_z, data = df)
    s <- summary(m)
    tibble::tibble(
      beta1 = unname(coef(m)["PGS_z"]),
      p = s$coefficients["PGS_z", "Pr(>|t|)"],
      r = cor(df$PGS_z, df$SBP, use = "complete.obs"),
...
# 2) 画图（仅一条全局回归线）+ 右上角标注
p_global <- dat_global %>%
  ggplot(aes(x = PGS_z, y = SBP, color = SuperPop, label = Population)) +
  geom_point(size = 1.5) +
  ...
  labs(x = "PGS (global z-score)", y = "Systolic blood pressure(mmHg)",
       title = "Global regression: Systolic blood pressure vs PGS (z, all populations combined)") +
  theme_bw()

ggsave(file.path(out_dir, "SBP_vs_pgs_global_annotated.png"),
...
for (sp in unique(dat_within$SuperPop)) {
  ...
  stats_within <- df_sp %>%
    ...
    dplyr::do({
      ...
      if (nrow(d) < 3) {
        ...
        m <- lm(SBP ~ PGS_z, data = d)
        s <- summary(m)
        tibble::tibble(
          beta1 = unname(coef(m)["PGS_z"]),
          p = s$coefficients["PGS_z", "Pr(>|t|)"],
          r = cor(d$PGS_z, d$SBP, use = "complete.obs"),
  ...
  p_sp <- df_sp %>%
    ggplot(aes(x = PGS_z, y = SBP, label = Population)) +
    geom_point(size = 1.5) +
    geom_text_repel(aes(label = Population), size = 2, max.overlaps = 50, show.legend = FALSE)+
    ...
    labs(x = paste0("PGS within ", sp, " (z-score)"),
         ...
         title = paste0("Within ", sp, ": Systolic blood pressure vs PGS (z)")) +
    theme_bw()
  
  ggsave(file.path(out_dir, paste0("SBP_vs_pgs_within_", sp, "_annotated.png")),
