...
bmi_csv <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/NCD_RisC_Lancet_2024_BMI_age_standardised_country.csv"
map_path  <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/trait/pop_to_country.tsv"
out_dir   <- "/Users/li/Desktop/UTAustin/Lab_Rotation/Arbel_Lab/Project/results_bmi_pgs"
...
bmi_raw <- read_csv(bmi_csv, show_col_types = FALSE)

bmi_tidy <- bmi_raw %>%
  rename(Country = `Country/Region/World`) %>%
  filter(Year == 2013) %>%
  select(Country, Sex, BMI_obesity = `Prevalence of BMI>=30 kg/m² (obesity)`) %>%
  mutate(
    Sex = recode(Sex,
                 "Men" = "Boys",
                 "Women" = "Girls")   # 统一命名风格
...
dat <- pgs_by_sex %>%
  inner_join(bmi_tidy, by = c("Country", "Sex")) %>%
  select(Sex, SuperPop, Population, Country, Mean_PGS, BMI_obesity)
...
# 定义一个通用回归函数
fit_one <- function(df) {
  model <- lm(BMI_obesity ~ PGS_z, data = df)
  s <- summary(model)
  tibble(
    n = nrow(df),
...
# 保存
write_csv(res_all, file.path(out_dir, "bmi_pgs_regression_results.csv"))
...
stats_global <- dat_global %>%
  ...
  dplyr::do({
    df <- .
    m <- lm(BMI_obesity ~ PGS_z, data = df)
    s <- summary(m)
    tibble::tibble(
      beta1 = unname(coef(m)["PGS_z"]),
      p = s$coefficients["PGS_z", "Pr(>|t|)"],
      r = cor(df$PGS_z, df$BMI_obesity, use = "complete.obs"),
...
# 2) 画图（仅一条全局回归线）+ 右上角标注
p_global <- dat_global %>%
  ggplot(aes(x = PGS_z, y = BMI_obesity, color = SuperPop, label = Population)) +
  geom_point(size = 1.5) +
  ...
  labs(x = "PGS (global z-score)", y = "Mean BMI (kg/m²)",
       title = "Global regression: BMI vs PGS (z, all populations combined)") +
  theme_bw()

ggsave(file.path(out_dir, "BMI_vs_pgs_global_annotated.png"),
...
for (sp in unique(dat_within$SuperPop)) {
  ...
  stats_within <- df_sp %>%
    ...
    dplyr::do({
      ...
      if (nrow(d) < 3) {
        ...
        m <- lm(BMI_obesity ~ PGS_z, data = d)
        s <- summary(m)
        tibble::tibble(
          beta1 = unname(coef(m)["PGS_z"]),
          p = s$coefficients["PGS_z", "Pr(>|t|)"],
          r = cor(d$PGS_z, d$BMI_obesity, use = "complete.obs"),
  ...
  p_sp <- df_sp %>%
    ggplot(aes(x = PGS_z, y = BMI_obesity, label = Population)) +
    geom_point(size = 1.5) +
    geom_text_repel(aes(label = Population), size = 2, max.overlaps = 50, show.legend = FALSE)+
    ...
    labs(x = paste0("PGS within ", sp, " (z-score)"),
         y = "Mean BMI (kg/m²)",
         title = paste0("Within ", sp, ": BMI vs PGS (z)")) +
    theme_bw()
  
  ggsave(file.path(out_dir, paste0("BMI_vs_pgs_within_", sp, "_annotated.png")),
